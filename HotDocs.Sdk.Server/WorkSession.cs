...
namespace HotDocs.Sdk.Server
{
	...
	/// <param name="options">The AssemblyOptions that will be used for this specific document assembly. This is
	/// a copy of the WorkSession's DefaultAssemblyOptions; if modified it affects only the current assembly.</param>
	...
	public delegate void PreAssembleDocumentDelegate(Template template, AnswerCollection answers, AssembleDocumentSettings options, object userState);
	...
	public class WorkSession
	{
		...
		/// all the interviews & assembling all the documents that may result from the given template.
		/// </summary>
		...
		public WorkSession(IServices service, Template template, TextReader answers)
		{
			...
			DefaultAssemblyOptions = new AssembleDocumentSettings();
		...
		/// <summary>
		/// When you create a WorkSession, a copy of the application-wide default assembly options (as specified
		...
		/// customized options.
		/// </summary>
		public AssembleDocumentSettings DefaultAssemblyOptions { get; private set; }

		/// <summary>
		/// The intent with DefaultInterviewOptions was to work much like DefaultAssemblyOptions. When you create
		/// a WorkSession, a copy of the application-wide default interview options (as specified in web.config) is
		...
		/// options.  HOWEVER, this doesn't work currently because InterviewWorkItems do not carry a reference
		...
		/// </summary>
		public InterviewSettings DefaultInterviewOptions { get; private set; }
		...
		/// <summary>
		/// Exposees a list of interview & document work items, both already completed and pending, as suitable for
		...
		/// an interview is current both before & after it has been presented to the user, all the way
		...
		/// TODO: include a table that shows the relationship between members of Document, AssemblyResult, WorkSession & DocumentWorkItem.
		/// </remarks>
			...
			PostAssembleDocumentDelegate postAssembleDocument, object userState, string logRef)
		{
			...
			while (workItem != null && workItem is DocumentWorkItem)
			{
				...
				// make a copy of the default assembly options and pass it to the BeforeAssembleDocumentDelegate (if provided)
				AssembleDocumentSettings asmOpts = new AssembleDocumentSettings(DefaultAssemblyOptions);
		...
		{
			InterviewSettings s = DefaultInterviewOptions;
			...
			return GetCurrentInterview(s);
		}

		public InterviewResult GetCurrentInterview()
		{
			return GetCurrentInterview(DefaultInterviewOptions);
		}

		public InterviewResult GetCurrentInterview(InterviewSettings options, IEnumerable<string> markedVariables = null, string logRef = "")
		{
			WorkItem currentWorkItem = CurrentWorkItem;
			TextReader answers = new StringReader(AnswerCollection.XmlAnswers);
			return _service.GetInterview(currentWorkItem.Template, answers, options, markedVariables, logRef);
		}
		...
		private void InsertNewWorkItems(Template[] templates, int parentPosition)
		{
