...
namespace HotDocs.Sdk.Server
{
    ...
    public class Document : IDisposable
    {
        ...
        internal Document(Template source, Stream document, DocumentType docType, NamedStream[] supportingFiles,
            string[] unansweredVariables)
        {
            if (docType == DocumentType.Native)
                throw new InvalidOperationException(
                    "DocumentResults and AssemblyResults must specify a DocumentType (if known).");
        ...
        public string ContentType
        {
            get
            {
                switch (Type)
                {
                    ...
                    case DocumentType.HPD:
                        return "application/x-hotdocs-hpd";
                    case DocumentType.HFD:
        ...
        {
            var docExtension = Path.GetExtension(docPath).Trim('.').ToLower();
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!disposed)
            {
                if (disposing)
                {
                    ...
                    {
                        foreach (var ns in SupportingFiles)
                            ns.Dispose();
