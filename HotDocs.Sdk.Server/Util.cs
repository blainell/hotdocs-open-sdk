...
{
    /// <summary>
    ///     The <c>Util</c> class provides some methods needed for this class. Some of those methods are used for a specific
    ///     implementation of IServices.
    /// </summary>
    public static class Util
    {
        ...
        internal static void ParseHdAsmCmdLine(string cmdLine, out string path, out string switches)
        {
            ...
            if (cmdLine.Length > 0)
            {
                ...
                var switchStart = cmdLine.IndexOf('/');
                if (switchStart == -1)
                {
                    path = cmdLine;
                }
        ...
        {
            var decryptedString = UtilityTools.DecryptString(templateLocator);
            var separatorIndex = decryptedString.IndexOf('|');
        ...
        internal static string ExtractString(BinaryObject obj)
        {
            ...
            if (obj.DataEncoding != null)
            {
                try
                {
                    enc = Encoding.GetEncoding(obj.DataEncoding);
                }
        ...
        internal static void AppendSdkScriptBlock(StringBuilder interview, Template template, InterviewSettings settings)
        {
            ...
            {
                interview.AppendFormat("HDOutlineInOrder={0};",
                    settings.NextFollowsOutline == Tristate.True ? "true" : "false");
                interview.AppendLine();
            }
            ...
            {
                interview.AppendFormat("HDShowAllResourceBtns={0};",
                    settings.ShowAllResourceButtons == Tristate.True ? "true" : "false");
                interview.AppendLine();
            }
            ...
            if (!string.IsNullOrEmpty(settings.AnswerFileDataServiceUrl) || (settings.CustomDataSources != null))
            {
                ...
                string address;
                foreach (var dataSource in templateManifest.DataSources)
                {
                    address = null;
        ...
        internal static string GetInterviewDefinitionUrl(InterviewSettings settings, Template template)
        {
            ...
            var url = settings.InterviewFilesUrl;
            url += url.Contains("?") ? "&" : "?";
        ...
        internal static string EmbedImagesInURIs(string fileName)
        {
            ...
            var targetFilenameNoExtention = Path.GetFileName(fileName).Replace(Path.GetExtension(fileName), "");
            // Iterates looking for images associated with the html file requested.
            foreach (var img in Directory.EnumerateFiles(Path.GetDirectoryName(fileName)))
            {
                var ext = Path.GetExtension(img).ToLower().Remove(0, 1);
                    ...
                    ((ext == "jpg") || (ext == "jpeg") || (ext == "gif") || (ext == "png") || (ext == "bmp")))
                {
                    ...
                    var data = File.ReadAllBytes(img);
                    // Replace the html src attribute that points to the image file to its base64 content
                    html = html.Replace(@"src=""" + Path.GetFileName(img),
                        @"src=""data:" + Sdk.Util.GetMimeType(img) + ";base64," + Convert.ToBase64String(data));
                }
            }
            return html;
        }

        internal static string CreateMimePart(string contentBoundary, string contentType, string contentID,
            string content, string transferEncoding)
        {
        ...
        internal static string HtmlToMultiPartMime(string htmlFileName)
        {
            ...
            var imageFilesExpression = @"src=""(.+[.](png|jpg|jpeg|jpe|jfif|gif|bmp|tif|tiff|wmf|ico))[""]";

            // Extracts and maps image files to mime multipart sections
            var imageMapping = (from Match m in Regex.Matches(html, imageFilesExpression, RegexOptions.IgnoreCase)
                select m.Groups[1].Value).Distinct()
                .ToDictionary(v => v,
                    v => Regex.Replace(Guid.NewGuid().ToString(), "{|}|-", "") + Path.GetExtension(v).ToLower());
            ...
            // Writes the header
            var boundary = "------=_NextPart_" + Regex.Replace(Guid.NewGuid().ToString(), "{|}", "");
            ...
            foreach (var pair in imageMapping)
            {
                result.Append(CreateMimePart(boundary,
                    Sdk.Util.GetMimeType(Path.GetExtension(pair.Key)),
                    "<" + pair.Value + ">",
        ...
        /// <summary>
        ///     This method returns the requested runtime file from the ServerFiles cache. If the file can be found in either the
        ///     cache or the
        ///     source URL, it is returned in the response. Otherwise, nothing is done with the response and the method returns
        ...
        public static Stream GetInterviewRuntimeFile(string fileName, string cacheFolder, string sourceUrl,
            out string contentType)
        {
            ...
            try
            {
                ...
                var cachedFilePath = Path.Combine(cacheFolder, fileName.Replace('/', '\\'));
                var cachedFileDirectory = Path.GetDirectoryName(cachedFilePath);
                ...
                var sourceFileUrl = sourceUrl.TrimEnd('/') + "/" + fileName;
                if (sourceFileUrl.IndexOf("files.hotdocs.ws") >= 0)
                {
                    var lastForwardSlash = sourceFileUrl.LastIndexOf('/');
                    sourceFileUrl = sourceFileUrl.Substring(0, lastForwardSlash).ToLower() +
                                    sourceFileUrl.Substring(lastForwardSlash);
                }
                ...
                if (File.Exists(cachedFilePath))
                {
                    ...
                    var fInfo = new FileInfo(cachedFilePath);
                    if (fInfo.LastWriteTimeUtc < DateTime.UtcNow.AddDays(-1))
                    {
                        ...
                        myRequest = WebRequest.Create(sourceFileUrl);
                        myRequest.Method = "HEAD";

                        using (var myResponse = myRequest.GetResponse())
                        {
                            var lastMod = myResponse.Headers["Last-Modified"];
                            var serverFileDate = DateTime.Parse(lastMod);
                ...
                if (!File.Exists(cachedFilePath))
                {
                    ...
                    using (var myResponse = myRequest.GetResponse())
                    {
                        using (var writeStream = new FileStream(cachedFilePath, FileMode.Create, FileAccess.Write))
                        ...
                        File.SetCreationTimeUtc(cachedFilePath,
                            DateTime.Parse(myResponse.Headers["Last-Modified"]).ToUniversalTime());
                    }
                }

                contentType = Sdk.Util.GetMimeType(cachedFilePath);
                if (File.Exists(cachedFilePath))
                {
        ...
        /// <summary>
        ///     This method is used by both WS and Cloud implementations of AssembleDocument to convert an AssemblyResult to an
        ...
        internal static AssembleDocumentResult ConvertAssemblyResult(Template template, AssemblyResult asmResult,
            DocumentType docType)
        {
            ...
            for (var i = 0; i < asmResult.Documents.Length; i++)
            {
                switch (asmResult.Documents[i].Format)
                {
                    ...
                    case OutputFormat.PNG:
                        ...
                        supportingFiles.Add(new NamedStream(asmResult.Documents[i].FileName,
                            new MemoryStream(asmResult.Documents[i].Data)));
                        break;
                    default:
            ...
            result = new AssembleDocumentResult(document == null
                ? null
