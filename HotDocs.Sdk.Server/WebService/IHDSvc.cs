...
namespace HotDocs.Sdk.Server.WebService
{
	...
	public interface IHDSvc
	{
		...
		[OperationContract]
		Contracts.TemplateInfo[] GetTemplateInfo(string[] templateIDs);
		...
		*/
		[OperationContract]
		BinaryObject[] GetInterview(string templateID, BinaryObject[] answers, DataContracts.InterviewFormat format, DataContracts.InterviewOptions options,
		...
		[OperationContract]
		BinaryObject GetInterviewDefinition(string templateID, string templateName, DataContracts.InterviewFormat format, string templateState);
		...
		[OperationContract]
		AssemblyResult AssembleDocument(string templateID, BinaryObject[] answers, DataContracts.OutputFormat format, AssemblyOptions options,
		...
		/// <param name="buildFlags">Specifies which files (besides the manifest) to build. See HDServerBuildFlags for details.</param>
		/// <param name="templateState">This parameter is either null if the template is located in the web service's default template
		/// directory (as specified in web.config), or it is an encrypted value encapsulating the location of the template.</param>
		[OperationContract]
		void BuildSupportFiles(string templateID, string templateKey, DataContracts.HDServerBuildFlags buildFlags, string templateState);
