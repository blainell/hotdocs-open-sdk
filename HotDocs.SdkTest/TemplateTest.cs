...
namespace HotDocs.SdkTest
{
    ...
    public class TemplateTest
    {
        public TemplateTest()
        {
            TemplateLocation.RegisterLocation(typeof (PathTemplateLocation));
        ...
        {
            var packageID = "d1f7cade-cb74-4457-a9a0-27d94f5c2d5b";
            var templateFileName = "Demo Employment Agreement.docx";
            var location = CreatePackagePathLocation(packageID);
            Template template;
            ...
            var filePath = template.GetFullPath();
        ...
        public void TestPathTemplateLocation()
        {
            ...
            try
            {
                TemplateLocation.RegisterLocation(typeof (string));
                Assert.Fail();
            }
            catch (Exception ex)
            {
            ...
            var testTemplates = new Dictionary<string, TemplateInfo>();
        ...
        private void TestTemplate(Template template, TemplateInfo tInfo)
        {
            ...
            var locator = template.CreateLocator();
            var template2 = Template.Locate(locator);
            ...
            Assert.AreEqual(".hfd", Template.GetDocExtension(DocumentType.HFD, null));
            Assert.AreEqual(".hpd", Template.GetDocExtension(DocumentType.HPD, null));
            Assert.AreEqual(".htm", Template.GetDocExtension(DocumentType.HTML, null));
            Assert.AreEqual(".htm", Template.GetDocExtension(DocumentType.HTMLwDataURIs, null));
            Assert.AreEqual(".htm", Template.GetDocExtension(DocumentType.MHTML, null));
            Assert.AreEqual(".pdf", Template.GetDocExtension(DocumentType.PDF, null));
            Assert.AreEqual(".txt", Template.GetDocExtension(DocumentType.PlainText, null));
            Assert.AreEqual(".doc", Template.GetDocExtension(DocumentType.WordDOC, null));
            Assert.AreEqual(".docx", Template.GetDocExtension(DocumentType.WordDOCX, null));
            Assert.AreEqual(".wpd", Template.GetDocExtension(DocumentType.WordPerfect, null));
            Assert.AreEqual(".rtf", Template.GetDocExtension(DocumentType.WordRTF, null));
            //Assert.AreEqual(".xml", HotDocs.Sdk.Template.GetDocExtension(DocumentType.XML, null)); // TODO: Should we be testing XML?

            // Unknown doc type.
            try
            {
                Assert.AreEqual(".unk", Template.GetDocExtension(DocumentType.Unknown, null));
            ...
            if (tInfo.DocType != DocumentType.Unknown)
            {
                ...
                Assert.AreEqual(tInfo.DocExtension, Template.GetDocExtension(DocumentType.Native, template));
            }
        ...
        public void TestTemplateLocationEquatability()
        {
            ...
            var loc = TemplateLocation.Locate(Util.EncryptString("abcdefg"));
            Assert.IsNull(loc);
            ...
            var pkgloc1a = new PackagePathTemplateLocation(@"2f98a9f3-d106-44a8-ba16-e01d8cd65cbd",
                Path.Combine(dir1, "temp.pkg"));
            var pkgloc1b = new PackagePathTemplateLocation(@"2f98a9f3-d106-44a8-ba16-e01d8cd65cbd",
                Path.Combine(dir1, "TEMP.pkg"));
            var pkgloc2a = new PackagePathTemplateLocation(@"2f98a9f3-d106-44a8-ba16-e01d8cd65cbD",
                Path.Combine(dir1, "TEMP.pkg"));
