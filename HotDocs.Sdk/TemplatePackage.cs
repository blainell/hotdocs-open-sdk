...
{
    /// <summary>
    ///     A class for HotDocs template packages. A template package is a zip file containing a template and all it's
    ///     dependent files. Optionally, a package can also be encrypted.
    /// </summary>
    public class TemplatePackage
    {
        /// <summary>
        ///     The internal name of the manifest file. It will be automatically created if the Manifest property has been set upon
        ///     saving the package. When opening a package, this manifest will be used to initialize the Manifest property.
        /// </summary>
        ...
        public bool IsNew
        {
            get { return string.IsNullOrEmpty(CurrentPath); }
        }
        ...
        {
            get { return Manifest == null ? null : Manifest.ToXml(); }
            set { Manifest = string.IsNullOrEmpty(value) ? null : TemplatePackageManifest.FromXml(value); }
        }
        ...
        /// </summary>
        /// <returns>
        ///     An XML string representing a new RSA public/private key pair defined in
        ...
        /// <returns>
        ///     If rsaParamsXml is null or empty, the input stream ms is simply returned. Otherwise, the input stream ms is
        ///     being copied and encrypted to the returned output stream. The output stream will contain a 4 byte signature at the
        ///     beginning identifying it as a HotDocs package followed by encrypted AES keys and the encrypted data.
        /// </returns>
        public static MemoryStream Encrypt(MemoryStream ms, string rsaParamsXml)
        {
            ...
            {
                aes.GenerateKey();
                decryptedAesKey = aes.Key;
                aes.GenerateIV();
                decryptedAesIV = aes.IV;
                using (var rsa = new RSACryptoServiceProvider())
                {
                    rsa.FromXmlString(rsaParamsXml);
                ...
                using (var cs = new CryptoStream(temps, aes.CreateEncryptor(), CryptoStreamMode.Write))
                {
                    ms.Position = 0;
                    CopyStream(cs, ms);
                    cs.FlushFinalBlock();
        ...
        /// <param name="rsaParamsXml">RSA key serialized to XML.</param>
        /// <returns>
        ///     If rsaParamsXml is null or empty or if the input stream ms is not an encrypted HotDocs package, the input
        ///     stream ms is simply returned. Otherwise, the input stream ms is being copied and decrypted to the returned output
        ///     stream.
        /// </returns>
        public static MemoryStream Decrypt(MemoryStream ms, string rsaParamsXml)
        {
            ...
            using (var aes = new AesCryptoServiceProvider())
            {
                ...
                var temps = new MemoryStream();
                using (var cs = new CryptoStream(temps, aes.CreateDecryptor(), CryptoStreamMode.Write))
                {
                    CopyStream(cs, ms);
                    cs.FlushFinalBlock();
                    cs.Flush();
        ...
        {
            var partUri = InternalNameToPartUri(ManifestName);
        ...
        private void SaveManifest()
        {
            ...
            using (var ps = packagePart.GetStream(FileMode.Create, FileAccess.ReadWrite))
            {
        ...
        /// <summary>
        ///     Save the package to a file. This function only works if Open has been called before and it uses the same file path
        ///     again.
        /// </summary>
        ...
        public void Save(string rsaParamsXml)
        {
            ...
            using (var fs = new FileStream(CurrentPath, FileMode.Create, FileAccess.ReadWrite))
            {
                ms.Position = 0;
                CopyStream(fs, ms);
            }
        ...
        /// <summary>
        ///     Save the package unencrypted to a file. This function only works if Open has been called before and it uses the
        ///     same file path again.
        /// </summary>
        public void Save()
        {
            Save(null);
        ...
        private bool CollectDependencies(TemplateInfo ti, Dictionary<string, Dependency> dict, out string errMsg)
        {
            ...
            var dictD = new Dictionary<string, Dependency>();
            //
            ...
            foreach (var dep in ti.Dependencies)
            {
                ...
                {
                    errMsg = "Dependency (Target=" + name + ", type=" + depType + ") is defined more than once";
                    return false;
                }
                dictD[key] = dep;
            }
        ...
        private bool IsValidTemplateInfo(TemplateInfo ti, Dictionary<string, Dependency> dict, out string errMsg)
        {
            ...
            {
                errMsg = "EffectiveComponentFile defines the file '" + ti.EffectiveComponentFile +
            ...
            foreach (var s in ti.GeneratedFiles)
            {
                ...
                if (ext == ".js" || ext == ".hvc")
                {
                    ...
                    if (dict.ContainsKey(key))
                    {
                        errMsg = "GeneratedFiles contains the file '" + name +
                                 "' which is already referenced from another template";
                        return false;
                    }
        ...
        {
            var dict = new Dictionary<string, Dependency>();
            ...
            for (i = 0; i < manifest.AdditionalFiles.Length; i++)
            {
                ...
                if (dict.ContainsKey(key))
                {
                    errMsg = "Additional file '" + name +
                             "' in manifest is already referenced from a template or occurs twice in additional files";
                    return false;
                }
                dict[key] = new Dependency(name, DependencyType.NoDependency);
                if (!FileExists(name))
        ...
        private static Uri InternalNameToPartUri(string internalName)
        {
            ...
            try
            {
                var paramUri = new Uri(internalName, UriKind.Relative);
        ...
        {
            var s = Uri.UnescapeDataString(partUri.ToString());
            //
        ...
        /// <param name="internalName">The name within the package. E.g. "template.rtf" or "images/icon.png".</param>
        /// <returns>
        ///     The internal name used. E.g. "template.rtf" or "images/icon.png". This name can be used to be shown to the
        ///     user and can be used to extract a file again.
        /// </returns>
        public string AddFile(string filePath, string internalName)
        {
            ...
            {
                Debug.WriteLine("TemplatePackage.AddFile Exception thrown: " + ex);
                throw;
            }
            if (packagePart != null)
        ...
        /// <param name="filePath">The full path of the file to be added. E.g. "D:\files\template.rtf".</param>
        /// <returns>
        ///     The internal name used. E.g. "template.rtf". This name can be used to be shown to the user and can be used to
        ///     extract a file again.
        /// </returns>
        ...
        /// <summary>
        ///     Get a list of all file names stored in the package. These are internal names. E.g. "template.rtf" or
        ///     "images/icon.png".
        /// </summary>
        ...
        ///     The folder where the file should be extracted to. This folder will be created if needed.
        ...
        ///     The full file path were the package should be written to. Any existing file will be
        ///     overwritten.
        /// </param>
        ...
        /// <param name="rsaParamsXml">
        ///     RSA key serialized to XML. It can be a public/private key pair or only a public key. If
        ///     null, save an unencrypted package.
        /// </param>
        ...
        public static void Create(string packagePath, TemplatePackageManifest manifest, IEnumerable<string> filePaths,
            string rsaParamsXml, CompressionOption compression, bool checkForGeneratedFiles)
        {
        ...
        public static void Create(string packagePath, TemplatePackageManifest manifest, IEnumerable<string> filePaths,
            string rsaParamsXml, CompressionOption compression)
        {
        ...
        /// </param>
        public static void Create(string packagePath, TemplatePackageManifest manifest, IEnumerable<string> filePaths,
            string rsaParamsXml, bool checkForGeneratedFiles)
        {
