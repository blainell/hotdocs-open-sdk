...
namespace HotDocs.Sdk
{
    /// <summary>
    ///     <para>
    ///         <c>TemplateLocation</c> is an abstract class serves as the base class for all template location objects.
    ...
    ///     </para>
    ///     <para>
    ///         The <c>TemplateLocation</c> class also serves as a dependency injection container for any
    ...
    ///         <c>TemplateLocation.RegisterLocation</c>.
    ///     </para>
    /// </summary>
    public abstract class TemplateLocation : IEquatable<TemplateLocation>
    {
        ...
        private static readonly ConcurrentQueue<Type> _registeredTypes = new ConcurrentQueue<Type>();
        ...
        /// <remarks>
        ///     <para>
        ///         In a system where all template file names are unique, one directory
        ///         may be used for all templates. Generally, however, each main template (e.g. a package's main template) should
        ///         reside in a directory separate from other main templates. Each dependent template (e.g. a package template that
        ///         is not the
        ///         main template) should reside in the same directory as its main template.
        ///     </para>
        ///     <para>
        ///         A <c>TemplateLocation</c> class may be implemented by the host application, and may represent templates
        ///         stored on the file system, in a DMS, in some other database, etc. However, when an actual template file path
        ///         is needed, the <c>TemplateLocation</c> implementation is expected to provide a full path to the template
        ///         directory, and
        ///         ensure that the template itself and all of its dependencies exist in that directory.
        ///     </para>
        ///     <para>
        ///         The directory where the template exists should survive the serialization and deserialization of this object.
        ///         If the file name needs to be updated at deserialization, the class should override the
        ///         <see cref="GetUpdatedFileName" />
        ///         method.
        ///     </para>
        /// </remarks>
        /// <returns></returns>
        ...
        /// <summary>
        ///     Create a <c>TemplateLocation</c> object from an encrypted locator string returned by
        ///     <c>TemplateLocation.CreateLocator</c>.
        /// </summary>
        ...
        {
            var locator = Util.DecryptString(encodedLocator);
            var stampLen = locator.IndexOf('|');
            if (stampLen == -1)
                return null;
            var stamp = locator.Substring(0, stampLen);
            var content = locator.Substring(stampLen + 1, locator.Length - (stampLen + 1));
            ...
            foreach (var type in _registeredTypes)
            {
                if (stamp == type.ToString())
                {
                    ...
                    var templateLocation = obj as TemplateLocation;
            ...
            throw new Exception("The type " + stamp +
                                " is not registered as a TemplateLocation. Call TemplateLocation.RegisterLocation at application start-up.");
        }
