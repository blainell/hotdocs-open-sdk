...
namespace HotDocs.Sdk
{
	...
	public class InterviewSettings : Settings
	{
		...
		/// </summary>
		/// <remarks>
		/// <para>This property is equivalent to the FormActionUrl property in the HotDocs Server API and the
		/// FormActionUrl setting in Core Services.</para>
		...
		public string PostInterviewUrl
		{
			get
			{
				return GetSettingString("FormActionUrl");
			}
			set // private?
			{
				SetSettingString("FormActionUrl", value);
			}
		}
		...
		/// <para>This property is equivalent to the HotDocsJavaScriptUrl property in the HotDocs Server API
		/// and the "HotDocsJsUrl" setting in Core Services.</para>
		/// <para>As with all properties of GetInterviewOptions and AssemblyOptions, a default value for this property
		/// can be specified in your web app's web.config file.</para>
		/// </remarks>
		public string InterviewRuntimeUrl
		{
			get
			{
				return GetSettingString("HotDocsJsUrl");
			}
			set // private?
			{
				SetSettingString("HotDocsJsUrl", value);
			}
		}
		...
		public string DocumentPreviewUrl
		{
			get
			{
				return GetSettingString("DocPreviewUrl");
			}
			set
			{
				SetSettingString("DocPreviewUrl", value);
			}
		}
		...
		public string SaveAnswersUrl
		{
			get
			{
				return GetSettingString("SaveAnswersPageUrl");
			}
			set
			{
				SetSettingString("SaveAnswersPageUrl", value);
			}
		}
		...
		public Tristate AddHdMainDiv
		{
			get
			{
				return GetSettingTristate("AddHdMainDiv");
			}
			set
			{
				SetSettingString("AddHdMainDiv", TristateToString(value));
			}
		}
		...
		/// </listheader>
		/// <item><term>HotDocsJsUrl</term>
		/// <description>The URL from which the web browser should request the HotDocs interview
		...
		/// to serve these files itself (from its own domain name).</description></item>
		/// <item><term>HotDocsCssUrl</term>
		/// <description>The base URL from which the web browser should request the interview style sheet and related components.
		/// The setting itself designates the full URL to the "user" CSS style sheet (for styling JavaScript interviews).
		/// The default user style sheet is named "hdsuser.css", but customized style sheets can have any base file name.
		/// Browser interviews rely on this setting to derive URLs for several other style-related components:
		/// (1) For HotDocs 11 and later, a sprite-style image must be available at this same base URL (but with a .png file
		/// extension) containing all the interview images at expected offsets within the sprite;
		/// (2) For Silverlight interviews, a XAML style sheet will be requested from this same base URL (but with a .xaml file extension);
		/// (3) The "system" style sheet (hdssystem.css) must also be available at the same base URL (differing by file name).</description></item>
		/// <item><term>HotDocsImageUrl</term>
		/// <description>In HotDocs 11, this setting is ignored since interview images (used for all buttons, tree nodes, etc.)
		/// were replaced with a single sprite-style image at the same base URL as the user style sheet.
		/// For HotDocs 10.x interviews, this is the base URL from which each of the standard images will be requested.</description></item>
		/// <item><term>InterviewDefUrl</term>
		/// <description>The base URL from which the interview runtime (running in the browser) will request interview definition files.
		/// For JavaScript interviews, interview definitions are JavaScript files; for Silverlight interviews, interview
		...
		/// <description>If this URL is null or empty, users will not be able to request document previews from within browser interviews.
		/// Otherwise this URL should be the location of a handler from which a browser interview can request a document preview.</description></item>
		/// <item><term>SaveAnswersPageUrl</term>
		/// <description>The URL of a page within your host application that allows saving answers during an interview.
		/// If this URL is null or empty, HotDocs will not show a Save Answers button on the interview toolbar.
		/// Otherwise the browser will POST interview answers to this page in response to the user clicking the Save Answers button.
		/// The page should return an HTML response that will be displayed within an IFRAME in the interview.</description></item>
		/// <item><term>FormActionUrl</term>
		/// <description>The URL to which interview answers are posted when the user clicks the Finish button at the end of an interview.
		...
		/// threshhold before HotDocs gives up on a request.</description></item>
		/// <item><term>AddHdMainDiv</term>
		...
		/// </description></item>
		/// <item><term>TemplateTitleOverride</term>
		/// <description>By default, HotDocs interviews show a template's title in the interview toolbar.
		/// Use this setting to override the text shown in the toolbar.</description></item>
		/// <item><term>InterviewLocale</term>
		/// <description>HotDocs interviews are assumed to use the English (US) language by default.
		...
		/// on Browser Language Modules.</description></item>
		/// <item><term>OutlineInOrder</term>
		/// <description>"true" or "false"<br/>
		/// Corresponds to the "Next Dialog Follows Outline" entry on the "Navigate" menu within desktop HotDocs.
		/// When this setting is "true" (the default), dialogs are navigated in a top-down manner, with the Next command
		...
		/// to its parent.</description></item>
		/// <item><term>ShowAllResourceButtons</term>
		/// <description>"true" or "false"<br/>
		...
		/// option in desktop HotDocs.</description></item>
		/// <item><term>DisableAnswerSummary</term>
		/// <description>"true" or "false"<br/>
		/// HotDocs interviews allow users to request answer summaries (from the interview toolbar) by default.
		/// This option can remove the answer summary toolbar button.</description></item>
		/// <item><term>AsMaximumMultipleChoice</term>
		/// <description>No longer supported. This setting was used to set the maximum number of selected multiple choice options that
		/// could be displayed in an answer summary. HotDocs now shows ALL selected multiple choice options in answer summaries.</description></item>
		/// <item><term>AsNumberOfColumns</term>
		/// <description>"1" or "2"<br/>
		/// Answer summaries can be formatted in 1-column or 2-column format.</description></item>
		/// <item><term>AsIndentPrompts</term>
		/// <description>"true" or "false"<br/>
		/// Indents the left margin of each prompt with respect to dialog titles. Applicable only to 1-column answer summary layout.</description></item>
		/// <item><term>AsBulletPrompts</term>
		/// <description>"true" or "false"<br/>
		/// Precedes each prompt with a bullet character. Applicable only to 1-column answer summary layout.</description></item>
		/// <item><term>AsIndentAnswers</term>
		/// <description>"true" or "false"<br/>
		/// Indents the left margin of each answer with respect to its prompt. Applicable only to 1-column answer summary layout.</description></item>
		/// <item><term>AsBulletAnswers</term>
		/// <description>"true" or "false"<br/>
		/// Precedes each answer with a bullet character. Applicable only to 1-column answer summary layout.</description></item>
		/// <item><term>AsBorders</term>
		/// <description>"N", "P" or "S"<br/>
		/// Sets the border style for the answer summary table:  [N]one, [P]lain or [S]culpted. Applicable only to 2-column answer summary layout.</description></item>
		/// <item><term>AsPercentOfWidthForPrompt</term>
		/// <description>Recommended range of "30" to "70"<br/>
		/// Specifies the percentage of the total interview width reserved for prompts.</description></item>
		/// <item><term>UnansweredFormat</term>
		/// <description>"Nothing" / "Underscores" / "Asterisks" / "[Variable]" / "*** Variable ***"<br/>
		/// Dictates what to merge (by default) when a value for which no answer is available is merged into text within the interview.
		...
		/// For performance reasons, HotDocs Core Services may ignore values over a certain threshhold.</description></item>
		...
		/// For performance reasons, HotDocs Core Services may ignore values over a certain threshhold.</description></item>
		/// </list>
		/// <para>When you do not explicitly provide values for any setting, the default values specified
		/// in the HotDocs Core Services Administrative Portal will be used.</para>
		...
		public string Title
		{
			get
			{
				return GetSettingString("TemplateTitleOverride");
			}
			set
			{
				SetSettingString("TemplateTitleOverride", value);
			}
		}
		...
		public string Locale
		{
			get
			{
				return GetSettingString("InterviewLocale");
			}
			set
			{
				SetSettingString("InterviewLocale", value);
			}
		}
		...
		/// top-level dialog (skipping child dialogs, if any), and choosing Next from a child dialog always navigates back
		/// to its parent.
		/// <para>This property is also equivalent to the "Next button follows interview outline" setting
		/// in the HotDocs Server management console and the OutlineInOrder setting in Core Services.</para>
		/// </remarks>
		public Tristate NextFollowsOutline
		{
			get
			{
				return GetSettingTristate("OutlineInOrder");
			}
			set
			{
				SetSettingString("OutlineInOrder", TristateToString(value));
			}
		}
		...
		public Tristate ShowAllResourceButtons
		{
			get
			{
				return GetSettingTristate("ShowAllResourceButtons");
			}
			set
			{
				SetSettingString("ShowAllResourceButtons", TristateToString(value));
			}
		}
		...
		public Tristate DisableAnswerSummary
		{
			get
			{
				return GetSettingTristate("DisableAnswerSummary");
			}
			set
			{
				SetSettingString("DisableAnswerSummary", TristateToString(value));
			}
		}
		...
		/// The choices are
		/// <list type="table"><listheader><term>Value</term><description>Merged into Text</description></listheader>
		/// <item><term>Default</term><description>(whichever of the below is specified on the server)</description></item>
		/// <item><term>Nothing</term><description>(nothing)</description></item>
		/// <item><term>Underscores</term><description>____________</description></item>
		/// <item><term>Asterisks</term><description>***</description></item>
		/// <item><term>VarNameWithBrakets</term><description>[Variable Name]</description></item>
		/// <item><term>VarNameWithAsterisks</term><description>*** Variable Name ***</description></item>
		/// </list>
		/// </summary>
		/// <remarks>
		/// <para>The property corresponds to the "Unanswered variable placeholder" setting in the HotDocs Server
		/// management console and the UnansweredFormat setting in Core Services.</para>
		/// <para>NOTE: This property is ignored if the SDK is communicating with HotDocs Server (either locally or
		/// at a web service).  HotDocs Server always honors the setting specified in the HotDocs Server
		/// Management Console, and does not allow individual interview requests to override that default value.</para>
		/// </remarks>
		...
		/// </summary>
		/// <remarks>
		/// <para>The property corresponds to the "Allow individual component files to override this placeholder"
		/// setting in the HotDocs Server management console and the HonorCmpUnansFormat setting in Core Services.</para>
		/// <para>NOTE: This property is ignored if the SDK is communicating with HotDocs Server (either locally or
		/// at a web service).  HotDocs Server always honors the setting specified in the HotDocs Server
		/// Management Console, and does not allow individual interview requests to override that default value.</para>
		/// </remarks>
		...
		/// </summary>
		/// <remarks>
		/// <para>The property corresponds to the "Default date format" setting in the HotDocs Server
		/// management console and the DefaultDateFormat setting in Core Services.</para>
		/// <para>NOTE: This property is ignored if the SDK is communicating with HotDocs Server (either locally or
		/// at a web service).  HotDocs Server always honors the setting specified in the HotDocs Server
		/// Management Console, and does not allow individual interview requests to override that default value.</para>
		/// </remarks>
