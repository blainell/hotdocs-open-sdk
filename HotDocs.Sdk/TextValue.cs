...
{
    /// <summary>
    ///     The TextValue struct is used to represent text or string values in HotDocs. Since it's a struct, TextValue is a
    ///     value type
    ///     with value semantics. Instances of TextValue directly contain the relevant text, and therefore (unlike reference
    ///     types) are immutable.
    /// </summary>
    public struct TextValue : IValue, IComparable
    {
        private readonly bool _protect;
        private static Regex s_xmlEscaper;
        private static Regex s_xmlUnescaper;

        /// <summary>
        ///     Static (shared) instance of an unanswered TextValue.
        /// </summary>
        public static readonly TextValue Unanswered;

        /// <summary>
        ///     Static (shared) instance of an unanswered TextValue that should be protected from unintentional modification in the
        ///     interview UI.
        /// </summary>
        public static readonly TextValue UnansweredLocked;
        ...
        static TextValue()
        {
            Unanswered = new TextValue();
            UnansweredLocked = new TextValue(null, false);
            s_xmlEscaper = null;
        }
        ...
        internal static string XMLEscape(string text)
        {
            ...
            return s_xmlEscaper.Replace(text,
                delegate(Match m) { return string.Format("\\{0:x2}", Convert.ToInt32(m.Value[0])); });
        }
        ...
        internal static string XMLUnescape(string text)
        {
            ...
            return s_xmlUnescaper.Replace(text,
                delegate(Match m) { return ((char) Convert.ToInt32(m.Value.Substring(1), 16)).ToString(); });
        }
        ...
        public object ToType(Type conversionType, IFormatProvider provider)
        {
            ...
            switch (conversionType.FullName)
            {
                case "HotDocs.Sdk.TextValue":
                    return IsAnswered ? new TextValue(ToString(provider)) : Unanswered;
