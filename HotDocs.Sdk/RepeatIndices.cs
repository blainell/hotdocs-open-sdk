...
namespace HotDocs.Sdk
{
    ...
    internal static class RepeatIndices
    {
        ...
        internal static int[] SetBaseIndices(int[] indices, int[] newBaseIndices)
        {
            ...
            var result = new int[indices.Length];
            for (var i = 0; i < newBaseIndices.Length; i++)
                result[i] = newBaseIndices[i];
            for (var i = newBaseIndices.Length; i < indices.Length; i++)
                result[i] = indices[i];
            return result;
        }
        ...
        internal static bool AreEquivalent(int[] indices1, int[] indices2)
        {
            ...
            var longer = indices2.Length > indices1.Length ? indices2 : indices1;
            var shorter = indices2.Length > indices1.Length ? indices1 : indices2;
            var i = 0;
        ...
        internal static bool AreRelated(int[] indices1, int[] indices2)
        {
            ...
            var sharedLen = Math.Min(indices1.Length, indices2.Length);
            for (var i = 0; i < sharedLen; i++)
