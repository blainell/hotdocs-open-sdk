...
namespace HotDocs.Sdk
{
    ...
    public class AnswerCollection : IEnumerable<Answer>
    {
        ...
        /// <summary>
        ///     The title of the answer collection. When the answer collection is serialized as XML, the title appears as an
        ///     attribute of the root AnswerSet node.
        /// </summary>
        public string Title { get; set; }

        /// <summary>
        ///     The version number of the answer collection. When the answer collection is serialized as XML, this version number
        ///     appears as an attribute of the root AnswerSet node.
        /// </summary>
        ...
        /// <summary>
        ///     The number of answers in the answer collection. Because HotDocs variables may be repeated, each of these answers
        ///     may actually contain more than one value.
        /// </summary>
        ...
        public IEnumerator<Answer> GetEnumerator()
        {
            foreach (var answerBucket in _answers)
            {
                if (answerBucket != null)
                {
                    foreach (var answer in answerBucket.Values)
                        yield return answer;
                }
            }
        }
        ...
        /// <returns>
        ///     True if the answer collection contained an answer for the specified variable and it was successfully removed;
        ...
        ///     Indicates whether or not answers that are marked as "do not save" should be written to the
        ///     answer file.
        /// </param>
        ...
        {
            WriteFile(FilePath, writeDontSave);
        }
        ...
        public void ReadXml(Stream input)
        {
            using (var rdr = new StreamReader(input, true))
            {
                ReadXml(rdr);
            }
        }
        ...
        public void ReadXml(TextReader input)
        {
            Clear();
            OverlayXml(input);
        }
        ...
        public void OverlayXml(TextReader input)
        {
            ...
            using (var reader = XmlReader.Create(input, settings))
            {
                ...
                var repeatStack = RepeatIndices.Empty;
                ...
                switch (reader.Name)
                {
                    case "TextValue":
                        ...
                        if (unans || string.IsNullOrEmpty(s))
                            ans.InitValue(userModifiable ? TextValue.Unanswered : TextValue.UnansweredLocked,
                                repeatStack);
                        else
                            ans.InitValue(new TextValue(s, userModifiable), repeatStack);
                        break;
                    case "NumValue":
                        if (ans == null)
                        ...
                        s = reader.ReadElementContentAsString(reader.Name, "");
                        if (unans || string.IsNullOrEmpty(s))
                            ans.InitValue(userModifiable ? NumberValue.Unanswered : NumberValue.UnansweredLocked,
                                repeatStack);
                        else
                            ans.InitValue(new NumberValue(XmlConvert.ToDouble(s), userModifiable), repeatStack);
                        break;
                    case "DateValue":
                        if (ans == null)
                        ...
                        s = reader.ReadElementContentAsString(reader.Name, "");
                        if (unans || string.IsNullOrEmpty(s))
                            ans.InitValue(userModifiable ? DateValue.Unanswered : DateValue.UnansweredLocked,
                                repeatStack);
                        else
                        {
                    ...
                    case "TFValue":
                        ...
                        s = reader.ReadElementContentAsString(reader.Name, "");
                        if (unans || string.IsNullOrEmpty(s))
                            ans.InitValue(userModifiable ? TrueFalseValue.Unanswered : TrueFalseValue.UnansweredLocked,
                                repeatStack);
                        else
                        {
                            ...
                            ans.InitValue(
                                new TrueFalseValue(XmlConvert.ToBoolean(s.ToLowerInvariant()), userModifiable),
                                repeatStack);
                        }
                        break;
                    case "MCValue":
                        ...
                        {
                            ans.InitValue(
                                userModifiable ? MultipleChoiceValue.Unanswered : MultipleChoiceValue.UnansweredLocked,
                                repeatStack);
                        ...
                        else
                        {
                            ...
                            if (mcVals.Count == 0) // as good as unanswered
                                ans.InitValue(
                                    userModifiable
                                        ? MultipleChoiceValue.Unanswered
                                        : MultipleChoiceValue.UnansweredLocked, repeatStack);
                            else
                                ans.InitValue(new MultipleChoiceValue(mcVals.ToArray(), userModifiable), repeatStack);
                    ...
                    default: // anything else -- database values, clause library values, document text (span) values
                        ...
                        ans.InitValue(new UnknownValue(reader.ReadOuterXml()), repeatStack);
                        break;
                }
        ...
        ///     answer file.
        /// </param>
        public void WriteXml(Stream output, bool writeDontSave)
        {
        ...
        public void WriteXml(Stream output, bool writeDontSave, bool closeStream)
        {
        ...
        ///     answer file.
        /// </param>
        public void WriteXml(TextWriter output, bool writeDontSave)
        {
            ...
            var forInterview = true; // ensures the userModifiable and userExtendible attributes are output
            using (XmlWriter writer = new AnswerXmlWriter(output, settings, forInterview))
            {
                ...
                var answerEnumerator = GetEnumerator();
